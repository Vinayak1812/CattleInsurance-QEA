...
const SeniorAdminDashboard = ({ onLogout, user }) => {
  ...
  const fetchPendingPolicies = async () => {
    try {
      const response = await fetch('http://localhost:8080/api/policies/pending-senior');
  ...
  const handleApprove = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/policies/${selectedPolicy.applicationId}/senior-approve`, {
        method: 'POST',
        headers: {
      ...
      const data = await response.json();
      if (data.success) {
        alert('Policy approved successfully! Application number assigned.');
        setShowModal(false);
  ...
  const handleReject = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/policies/${selectedPolicy.applicationId}/senior-reject`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    <div className="admin-dashboard">
      ...
      <main className="admin-main">
        ...
        <div className="policies-section">
          <h2>Junior Admin Approved Applications</h2>
          {pendingPolicies.length === 0 ? (
            <p className="no-policies">No applications pending senior admin review</p>
          ) : (
              ...
              {pendingPolicies.map((policy) => (
                <div key={policy.applicationId} className="policy-card">
                  ...
                  <div className="policy-details">
                    ...
                    <p><strong>Junior Admin ID:</strong> {policy.juniorAdminId}</p>
                    <p><strong>Junior Admin Comments:</strong> {policy.comments}</p>
          ...
          <div className="modal">
            ...
            <div className="modal-body">
              <div className="policy-review-details">
                ...
                <p><strong>Junior Admin ID:</strong> {selectedPolicy.juniorAdminId}</p>
                <p><strong>Junior Admin Comments:</strong> {selectedPolicy.comments}</p>
